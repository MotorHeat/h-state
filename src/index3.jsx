...
function hookAction(fstate, actionDef) {
  return function (e) {
    if (Array.isArray(actionDef)) {
      ...
      runAction(fstate, actionDef[0], payload)
    } else {
      runAction(fstate, actionDef)
    }
  }
}

function runAction(fstate, action, payload) {
    let actionResult = action(fstate(), payload)
    if (Array.isArray(actionResult) && Array.isArray(actionResult[1])
      && typeof(actionResult[1][0]) === "function") {
      fstate(actionResult[0])
      actionResult[1][0](fstate, actionResult[1][1])
    } else {
      fstate(actionResult)
    }
}
